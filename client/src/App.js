...
const DailymotionIcon = ({ size = 18 }) => (
  <svg width={size} height={size} viewBox="0 0 32 32" fill={PLATFORM_BRAND.dailymotion.icon} aria-label="Dailymotion">
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 8px 12px', display: 'flex', flexDirection: 'column' }}>
        {[
          { title: 'AI Summaries', sub: 'Bullet point summaries', color: P.accent, bg: 'rgba(60,140,255,0.1)',
            ...
            onClick: summary ? () => { setActiveTab('summary'); setSidebarTab('summary'); setMobilePanel('summary'); setHistoryDrawerOpen(false); } : summarize, active: !!summary, loading: summarizing },
          { title: 'Flash Cards', sub: 'Q&A cards with flip mode', color: P.warning, bg: 'rgba(180,83,9,0.1)',
            ...
            onClick: flashcards.length > 0 ? () => { setActiveTab('flashcards'); setSidebarTab('flashcards'); setMobilePanel('flashcards'); setHistoryDrawerOpen(false); } : generateFlashcards, active: flashcards.length > 0, loading: flashcardsLoading },
          { title: 'Study Guide', sub: 'Objectives, concepts & review', color: P.success, bg: 'rgba(15,118,110,0.1)',
            ...
            onClick: studyGuide && !studyGuide._error ? () => { setActiveTab('study-guide'); setSidebarTab('study-guide'); setMobilePanel('study-guide'); setHistoryDrawerOpen(false); } : generateStudyGuide, active: !!studyGuide && !studyGuide._error, loading: studyGuideLoading },
          { title: 'Academic', sub: 'References, claims & glossary', color: '#7C3AED', bg: 'rgba(124,58,237,0.1)',
            ...
            onClick: academicInsights && !academicInsights._error ? () => { setActiveTab('academic'); setSidebarTab('academic'); setMobilePanel('academic'); setHistoryDrawerOpen(false); } : generateAcademicInsights, active: !!academicInsights && !academicInsights._error, loading: academicInsightsLoading },
          { title: 'Discover', sub: 'Related videos & papers', color: '#C2410C', bg: 'rgba(194,65,12,0.1)',
            ...
            onClick: discover && !discover._error ? () => { setActiveTab('discover'); setSidebarTab('discover'); setMobilePanel('discover'); setHistoryDrawerOpen(false); } : generateDiscover, active: !!discover && !discover._error, loading: discoverLoading },
        ].map(item => (
          ...
          <div style={{ animation: 'fadeUp 0.4s ease', paddingBottom: 64 }}>
            ...
            <div className="hero-inner" style={{
              ...
              <div style={{
                ...
                {(isMobile
                  ? (windowWidth < 390
                    ? ['Your own uploads', 'AI chat']
                    : ['Your own uploads', 'AI chat', 'Flashcards'])
                  : ['Instant extraction', 'Your own uploads', 'AI summaries', 'Flashcards', 'Study guides', 'AI chat']
                ).map((item, i) => (
                  <span key={item} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: isMobile ? 11 : 12, color: P.muted, flexShrink: 0 }}>
        ...
        {transcript && (
          ...
          <div className="fade-up" style={{
            ...
            <div style={{
              ...
              {currentVideoId && (
                <div style={{ flexShrink: 0, borderRadius: 16, overflow: 'hidden', border: `1px solid ${P.border}`, background: P.paper }}>
                  ...
                  {/* Slim meta bar */}
                  <div style={{ padding: isMobile ? '6px 12px' : '9px 14px', display: 'flex', alignItems: 'center', gap: 10, background: '#FFFFFF' }}>
                    <div style={{ minWidth: 0, flex: 1 }}>
              ...
              <div className="no-scrollbar" style={{ flexShrink: 0, display: 'flex', alignItems: 'flex-end', gap: 2, padding: '6px 10px 0', background: P.paper, borderBottom: `1px solid ${P.border}`, overflowX: isMobile ? 'auto' : 'visible', overflowY: 'hidden', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                ...
                {[
                  { key: 'transcript', label: 'Transcript', closeable: false, icon: <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><line x1="10" y1="9" x2="8" y2="9"/></svg> },
                  ...(!isMobile ? [{ key: 'editor', label: 'Editor', closeable: false, icon: <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg> }] : []),
                  ...(!isMobile && (flashcards.length > 0 || flashcardsExhausted) ? [{ key: 'flashcards', label: 'Flashcards', closeable: true, onClose: () => { setFlashcards([]); setFlashcardsExhausted(false); setFlashcardsExhaustedReason(''); if (activeTab === 'flashcards') setActiveTab('transcript'); }, icon: <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg> }] : []),
                  ...
                  return (
                    <button
                      ...
                      style={{
                        display: 'flex', alignItems: 'center', gap: 5,
                        padding: tab.closeable ? (isMobile ? '8px 8px 8px 12px' : '5px 6px 5px 12px') : (isMobile ? '8px 12px' : '5px 12px'),
                        minHeight: isMobile ? 44 : undefined, fontSize: 12,
                        fontWeight: isActive ? 600 : 500,
                        border: isActive ? '1px solid rgba(60,140,255,0.25)' : isOver ? `1px solid ${P.accent}` : '1px solid transparent',
                        borderBottom: isActive ? '1px solid #FFFFFF' : '1px solid transparent',
                        borderRadius: '7px 7px 0 0', marginBottom: '-1px',
                        background: isActive ? P.accentLight : isOver ? 'rgba(60,140,255,0.06)' : 'transparent',
                      ...
                      onMouseLeave={e => { if (!isActive) { e.currentTarget.style.background = isOver ? 'rgba(60,140,255,0.06)' : 'transparent'; e.currentTarget.style.color = P.muted; }}}
                    >
                      {tab.icon}
                      {tab.label}
                      {tab.closeable && (
                                  ...
                                  <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                                    {msg.role === 'ai' ? (
                                      <div style={{ maxWidth: '90%', padding: '9px 13px', borderRadius: '3px 12px 12px 12px', background: P.paper, border: `1px solid ${P.border}`, fontSize: 13, lineHeight: 1.65, color: msg.isError ? P.error : P.ink }}>
                                        {msg.text.split('\n').map((line, li, arr) => <React.Fragment key={li}>{renderChatLine(line)}{li < arr.length - 1 && <br />}</React.Fragment>)}
                                      </div>
                                    ) : (
            ...
            <div style={{
                        ...
                        {msg.role === 'ai' ? (
                          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, maxWidth: '93%' }}>
                            ...
                            <div style={{
                              ...
                              {msg.text.split('\n').map((line, li, arr) => (
                                <React.Fragment key={li}>{renderChatLine(line)}{li < arr.length - 1 && <br />}</React.Fragment>
                              ))}
                            </div>
                          </div>
                        ) : (
              ...
              {!isDesktop && sidebarTab === 'summary' && (
                <div style={{ flex: 1, overflowY: 'auto', padding: '20px 18px' }}>
                  {summarizing ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: P.ink }}>AI-generated summary</div>
                        <div style={{ display: 'flex', gap: 5 }}>
                          <button onClick={() => { navigator.clipboard.writeText(summary).then(() => { setSummaryCopied(true); setTimeout(() => setSummaryCopied(false), 2000); }); }}
                            style={{ display: 'flex', alignItems: 'center', gap: 4, border: `1px solid ${P.border}`, background: 'none', cursor: 'pointer', borderRadius: 6, padding: '4px 9px', fontSize: 11, fontWeight: 600, color: summaryCopied ? P.success : P.muted }}>
                            {summaryCopied ? <CheckIcon /> : <CopyIcon />} {summaryCopied ? 'Copied!' : 'Copy'}
                          </button>
                          <button onClick={() => { setSummary(''); setSidebarTab('insights'); }}
                  ...
                  {flashcardsLoading ? (
                    ...
                    <>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: P.ink }}>{flashcards.length} cards · {flashcardKnown.size} known</div>
                        {flashcards.length > 0 && (
                          <button onClick={() => { setFlashcardIndex(0); setFlashcardFlipped(false); setShowFlashcardModal(true); }}
                            style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 7, border: 'none', background: P.accent, color: 'white', fontSize: 11.5, fontWeight: 600, cursor: 'pointer' }}>
                      ...
                      {flashcardsExhausted && (
                        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '10px 12px', borderRadius: 10, border: '1px solid rgba(245,158,11,0.3)', background: 'rgba(245,158,11,0.07)', marginBottom: 10 }}>
                          ...
                          <div style={{ fontSize: 12, color: P.muted, lineHeight: 1.5 }}>{flashcardsExhaustedReason}</div>
                        </div>
                      )}
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {flashcards.map((card, i) => {
                          ...
                          const isKnown = flashcardKnown.has(i);
                          return (
                            <div key={i} style={{ borderRadius: 9, border: `1px solid ${isKnown ? 'rgba(15,118,110,0.3)' : P.border}`, background: isKnown ? 'rgba(15,118,110,0.04)' : '#fff', overflow: 'hidden' }}>
                              <div onClick={() => setExpandedCards(prev => { const next = new Set(prev); if (next.has(i)) next.delete(i); else next.add(i); return next; })}
                                ...
                                <div style={{ width: 22, height: 22, borderRadius: 5, background: isKnown ? 'rgba(15,118,110,0.12)' : P.surface, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                  <span style={{ fontSize: 9.5, fontWeight: 700, color: isKnown ? P.success : P.muted }}>{i + 1}</span>
                                </div>
                                <div style={{ flex: 1, minWidth: 0, fontSize: 12, fontWeight: 600, color: P.ink, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: isExpanded ? 'normal' : 'nowrap' }}>{card.question}</div>
                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={P.muted} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, transform: isExpanded ? 'rotate(180deg)' : 'rotate(0)', transition: 'transform 0.2s' }}><polyline points="6 9 12 15 18 9"/></svg>
                              </div>
                              {isExpanded && (
                                <div style={{ borderTop: `1px solid ${P.border}`, padding: '10px 12px', background: P.paper }}>
                                  {card.topic && <div style={{ fontSize: 10, fontWeight: 700, color: '#D97706', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{card.topic}</div>}
                                  <div style={{ fontSize: 12.5, lineHeight: 1.6, color: P.ink }}>{card.answer}</div>
                                  <button onClick={e => { e.stopPropagation(); setFlashcardKnown(prev => { const next = new Set(prev); if (isKnown) next.delete(i); else next.add(i); return next; }); }}
                                    style={{ marginTop: 8, width: '100%', padding: '5px 0', borderRadius: 6, border: `1px solid ${isKnown ? 'rgba(15,118,110,0.4)' : P.border}`, background: isKnown ? 'rgba(15,118,110,0.08)' : 'none', color: isKnown ? P.success : P.muted, fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
                      ...
                      {!flashcardsExhausted && (
                        <button onClick={generateMoreFlashcards} disabled={flashcardsMoreLoading}
                          style={{ marginTop: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, width: '100%', padding: '7px 0', borderRadius: 7, border: `1.5px dashed ${P.border}`, background: 'none', color: P.ink, fontSize: 12, fontWeight: 600, cursor: flashcardsMoreLoading ? 'default' : 'pointer', opacity: flashcardsMoreLoading ? 0.6 : 1 }}>
                          {flashcardsMoreLoading ? <><SpinnerIcon size={11} />Generating…</> : <>+ More Cards</>}
                        </button>
                      )}
                      {flashcardKnown.size > 0 && (
                        <button onClick={() => setFlashcardKnown(new Set())}
                          style={{ marginTop: 6, display: 'block', width: '100%', padding: '7px 0', borderRadius: 7, border: `1px solid ${P.border}`, background: 'none', color: P.muted, fontSize: 11.5, fontWeight: 600, cursor: 'pointer' }}>
                          Reset Progress
                        </button>
                      )}
                    </>
                    ...
                    return (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        ...
                        {sg.objectives && sg.objectives.length > 0 && (
                          <div>
                            <div style={{ fontSize: 12, fontWeight: 700, color: P.ink, marginBottom: 8 }}>Learning Objectives</div>
                            <ul style={{ margin: 0, paddingLeft: 18, display: 'flex', flexDirection: 'column', gap: 5 }}>
                              {sg.objectives.map((obj, i) => <li key={i} style={{ fontSize: 12.5, color: P.ink, lineHeight: 1.55 }}>{obj}</li>)}
                            </ul>
                          </div>
                        )}
                        {sg.keyConcepts && sg.keyConcepts.length > 0 && (
                          <div>
                            <div style={{ fontSize: 12, fontWeight: 700, color: P.ink, marginBottom: 8 }}>Key Concepts</div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                              {sg.keyConcepts.map((kc, i) => (
                                <div key={i} style={{ padding: '10px 12px', background: '#fff', borderRadius: 9, border: `1px solid ${P.border}` }}>
                                  <div style={{ fontSize: 12.5, fontWeight: 700, color: P.ink, marginBottom: 3 }}>{kc.concept}</div>
                                  <div style={{ fontSize: 12, color: P.muted, lineHeight: 1.5 }}>{kc.explanation}</div>
                                </div>
                              ))}
                            </div>
                          </div>
                        ...
                        {sg.sections && sg.sections.length > 0 && (
                          <div>
                            <div style={{ fontSize: 12, fontWeight: 700, color: P.ink, marginBottom: 8 }}>Sections</div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                              {sg.sections.map((sec, i) => (
                                <div key={i} style={{ padding: '12px 14px', background: '#fff', borderRadius: 10, border: `1px solid ${P.border}` }}>
                                  <div style={{ fontSize: 13, fontWeight: 700, color: P.ink, marginBottom: 5 }}>{sec.title}</div>
                                  {sec.points && sec.points.length > 0 && (
                                    <ul style={{ margin: 0, paddingLeft: 16, display: 'flex', flexDirection: 'column', gap: 3 }}>
                                      {sec.points.map((pt, j) => <li key={j} style={{ fontSize: 12, color: P.ink, lineHeight: 1.5 }}>{pt}</li>)}
                                    </ul>
                                  )}
                                </div>
                              ))}
                        ...
                        {sg.reviewQuestions && sg.reviewQuestions.length > 0 && (
                          <div>
                            <div style={{ fontSize: 12, fontWeight: 700, color: P.ink, marginBottom: 8 }}>Review Questions</div>
                            <ol style={{ margin: 0, paddingLeft: 18, display: 'flex', flexDirection: 'column', gap: 5 }}>
                              {sg.reviewQuestions.map((q, i) => <li key={i} style={{ fontSize: 12.5, color: P.ink, lineHeight: 1.55 }}>{q}</li>)}
                            </ol>
                          </div>
                        )}
                        <button onClick={() => { setStudyGuide(null); setSidebarTab('insights'); }}
                          style={{ marginTop: 4, border: `1px solid ${P.border}`, background: 'none', cursor: 'pointer', color: P.muted, fontSize: 11.5, fontWeight: 600, padding: '6px 14px', borderRadius: 7 }}>
                          Clear Study Guide
                        </button>
                      </div>
              ...
              {/* Discover sidebar tab — mobile/tablet only */}
              {!isDesktop && sidebarTab === 'discover' && (
                <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px 32px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                  {discoverLoading ? (
                    <div style={{ padding: '60px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, color: P.muted, fontSize: 13 }}>
                      <SpinnerIcon size={14} /> Finding related content…
                    </div>
                  ) : discover && !discover._error ? (
                    <>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 34, height: 34, borderRadius: 10, background: 'rgba(194,65,12,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#C2410C', flexShrink: 0 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 14, fontWeight: 700, color: P.ink }}>Discover</div>
                          <div style={{ fontSize: 11.5, color: P.muted }}>Related videos &amp; papers</div>
                        </div>
                        <button onClick={() => { setDiscover(null); setSidebarTab('insights'); }}
                          style={{ border: `1px solid ${P.border}`, background: 'none', cursor: 'pointer', color: P.muted, fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: 6 }}>Clear</button>
                      </div>
                      {discover.keywords?.length > 0 && (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                          {discover.keywords.map((kw, i) => (
                            <span key={i} style={{ fontSize: 11, fontWeight: 600, color: '#C2410C', background: 'rgba(194,65,12,0.08)', padding: '3px 8px', borderRadius: 99 }}>{kw}</span>
                          ))}
                        </div>
                      )}
                      {discover.videos?.length > 0 && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                          <div style={{ fontSize: 11, fontWeight: 700, color: P.muted, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Videos · {discover.videos.length}</div>
                          {discover.videos.map((v, i) => (
                            <a key={i} href={`https://www.youtube.com/watch?v=${v.id}`} target="_blank" rel="noopener noreferrer"
                              style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '10px 12px', borderRadius: 10, border: `1px solid ${P.border}`, background: P.surface, textDecoration: 'none', transition: 'all 0.15s' }}
                              onMouseEnter={e => { e.currentTarget.style.borderColor = P.accent; e.currentTarget.style.background = P.paper; }}
                              onMouseLeave={e => { e.currentTarget.style.borderColor = P.border; e.currentTarget.style.background = P.surface; }}
                            >
                              {v.thumbnail && <img src={v.thumbnail} alt="" style={{ width: 60, height: 34, borderRadius: 5, objectFit: 'cover', flexShrink: 0 }} />}
                              <div style={{ minWidth: 0, flex: 1 }}>
                                <div style={{ fontSize: 12.5, fontWeight: 600, color: P.ink, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{v.title}</div>
                                <div style={{ fontSize: 11, color: P.muted, marginTop: 2 }}>{v.channel}</div>
                              </div>
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={P.accent} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
                            </a>
                          ))}
                        </div>
                      )}
                      {discover.papers?.length > 0 && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                          <div style={{ fontSize: 11, fontWeight: 700, color: P.muted, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Papers · {discover.papers.length}</div>
                          {discover.papers.map((p, i) => (
                            <div key={i} style={{ padding: '12px 14px', borderRadius: 10, border: `1px solid ${P.border}`, background: P.surface, display: 'flex', flexDirection: 'column', gap: 5 }}>
                              <div style={{ fontSize: 13, fontWeight: 700, color: P.ink, lineHeight: 1.4 }}>{p.title}</div>
                              <div style={{ fontSize: 11, color: P.muted }}>{p.authors?.[0]}{p.authors?.length > 1 ? ' et al.' : ''}{p.year ? ` · ${p.year}` : ''}</div>
                              {p.abstract && <div style={{ fontSize: 12, color: P.muted, lineHeight: 1.55, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}>{p.abstract}</div>}
                              <div style={{ display: 'flex', gap: 6, marginTop: 2 }}>
                                {p.pdfUrl && <a href={p.pdfUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, fontWeight: 600, color: P.accent, background: P.accentLight, padding: '3px 9px', borderRadius: 6, textDecoration: 'none' }}>PDF</a>}
                                {p.doi && <a href={`https://doi.org/${p.doi}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, fontWeight: 500, color: P.muted, textDecoration: 'none' }}>DOI ↗</a>}
                                {!p.pdfUrl && !p.doi && p.paperId && <a href={`https://www.semanticscholar.org/paper/${p.paperId}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, fontWeight: 500, color: P.muted, textDecoration: 'none' }}>View ↗</a>}
                              </div>
                            </div>
                      ...
                      {!discover.videos?.length && !discover.papers?.length && (
                        <div style={{ padding: '32px 0', textAlign: 'center', color: P.muted, fontSize: 13 }}>No related content found.</div>
                      )}
                    </>
                  ) : discover?._error ? (
                    <div style={{ color: P.error, fontSize: 13 }}>Failed: {discover._error}</div>
                  ) : null}
                </div>
              )}
